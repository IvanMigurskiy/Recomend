...
for i in [x / 10.0 for x in range(1, 11, 1)]:
    ratings2 = ratings[0:int(len(ratings) * i)]

    train_size = int(train_pct * ratings2.shape[0])

    train = ratings2[:train_size]
    validation = ratings2[train_size:]
    ...
    pmf = PMF(n_user=n_user, n_item=n_item, n_feature=n_feature, n_size=int(len(ratings2)),
    ...
    print("bpmf n_user: %d, n_item: %d, n_feature: %d, training size: %d, validation size: %d" % (
        n_user, n_item, n_feature, train.shape[0], validation.shape[0]))
    bpmf = BPMF(n_user=n_user, n_item=n_item, n_feature=n_feature, n_size=int(len(ratings2)),
    ...
    print("als  n_user: %d, n_item: %d, n_feature: %d, training size: %d, validation size: %d" % (
        n_user, n_item, n_feature, train.shape[0], validation.shape[0]))
    als = ALS(n_user=n_user, n_item=n_item, n_feature=n_feature, n_size=int(len(ratings2)),
